  ...
  const startGame = async () => {
    ...
    try {
      const result = await getQuestions({
        domain: 'spot-difference',
        grade: parseInt(grade),
        count: 10,
        seed: `spot-difference-${Date.now()}`,
      })
        ...
        {questions.length === 0 ? (
          <div className="bg-white rounded-2xl shadow-lg border border-slate-200 p-8">
            ...
            <button
              onClick={startGame}
              disabled={isLoading}
              className="w-full py-4 bg-gradient-to-r from-emerald-500 to-green-500 text-white font-bold text-lg rounded-xl hover:opacity-90 disabled:opacity-50 transition-all"
            >
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center justify-center gap-4">
              ...
              <button
                ...
                className="px-6 py-3 bg-gradient-to-r from-emerald-500 to-green-500 text-white rounded-xl font-medium disabled:opacity-40 disabled:cursor-not-allowed hover:opacity-90 transition-all"
              >
                Келесі →
              </button>
              <button
