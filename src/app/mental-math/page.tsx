...
import { Calculator, ArrowLeft, Monitor, RotateCcw, Settings } from 'lucide-react'
import {
  GameQuestion,
  GameAnswer,
  GameResult,
...
export default function MentalMathPage() {
  ...
  const startGame = useCallback(() => {
    ...
    if (difficulty === 'medium') {
      gameQuestions = generateProgressiveQuestions(generateMentalMathQuestions, grade, 10)
    } else {
      gameQuestions = generateMentalMathQuestions(grade, difficulty, 10)
    }
    ...
    setTimeLeft(gameQuestions[0]?.timeLimit || 10)
  }, [grade, difficulty])
  ...
  const nextQuestion = useCallback(() => {
    if (currentIndex < questions.length - 1) {
      ...
      setIsAnswered(false)
      setQuestionStartTime(Date.now())
      setTimeLeft(questions[currentIndex + 1]?.timeLimit || 10)
    } else {
      const challenge = generateMathChallengeQuestion(grade)
  ...
  const finishGame = useCallback((finalChallengeAnswer?: GameAnswer) => {
    const gameResult = calculateGameResult(answers, 10, finalChallengeAnswer)
  ...
  return (
    <div className={`min-h-screen py-8 ${isClassroomMode ? 'bg-yellow-900' : 'bg-gradient-to-b from-yellow-100 to-orange-50'}`}>
      <div className={`mx-auto px-4 sm:px-6 lg:px-8 ${isClassroomMode ? 'max-w-6xl' : 'max-w-4xl'}`}>
        
        <Link href="/" className={`inline-flex items-center gap-2 mb-6 ${isClassroomMode ? 'text-yellow-200 hover:text-white' : 'text-slate-600 hover:text-slate-900'}`}>
        ...
        <div className="text-center mb-8">
          <div className="flex justify-center mb-4">
            <div className={`rounded-2xl bg-amber-500 flex items-center justify-center ${isClassroomMode ? 'w-24 h-24' : 'w-16 h-16'}`}>
              <Calculator className={`text-white ${isClassroomMode ? 'w-12 h-12' : 'w-8 h-8'}`} strokeWidth={1.75} />
            </div>
          </div>
          <h1 className={`font-bold mb-2 ${isClassroomMode ? 'text-5xl text-white' : 'text-4xl text-slate-900'}`}>
            Жылдам есеп
          </h1>
          <p className={isClassroomMode ? 'text-xl text-yellow-200' : 'text-slate-600'}>
            Ойлау жылдамдығын арттыру ойыны
          </p>
        </div>

        {/* Menu Phase */}
        {phase === 'menu' && (
          <div className={`rounded-2xl shadow-xl p-8 ${isClassroomMode ? 'bg-yellow-800 border-2 border-yellow-500' : 'bg-white border border-slate-200'}`}>
            ...
            <button
              onClick={startGame}
              className="w-full py-5 bg-gradient-to-r from-yellow-500 to-orange-500 text-white font-bold text-2xl rounded-xl hover:opacity-90 transition-all shadow-lg flex items-center justify-center gap-3"
            >
          ...
          <div className={`rounded-2xl shadow-xl p-8 ${isClassroomMode ? 'bg-yellow-800 border-2 border-yellow-500' : 'bg-white border border-slate-200'}`}>
            ...
            <div className="flex items-center justify-between mb-6">
              <div className={`text-lg font-bold ${isClassroomMode ? 'text-white' : 'text-slate-700'}`}>
                {phase === 'challenge' ? (
                  <span className="text-red-500">🔥 ЧЕЛЛЕНДЖ!</span>
                ) : (
                  <span>Есеп {currentIndex + 1} / {questions.length}</span>
                )}
              </div>
            ...
            <div className={`text-center mb-8 ${isClassroomMode ? 'py-8' : 'py-4'}`}>
              <p className={`font-bold leading-relaxed ${isClassroomMode ? 'text-6xl text-white' : 'text-4xl text-slate-900'}`}>
              ...
              {(phase === 'challenge' ? challengeQuestion?.options : currentQuestion?.options)?.map((option, idx) => {
                ...
                if (isAnswered) {
                  if (isCorrect) {
                    ...
                    buttonClass = isClassroomMode ? 'bg-yellow-700 text-yellow-300' : 'bg-slate-100 text-slate-400'
                  }
                } else {
                  buttonClass = isClassroomMode 
                    ? 'bg-yellow-600 text-white hover:bg-yellow-500 hover:scale-105' 
        ...
        {phase === 'results' && result && (
          <div className={`rounded-2xl shadow-xl p-8 ${isClassroomMode ? 'bg-yellow-800 border-2 border-yellow-500' : 'bg-white border border-slate-200'}`}>
            ...
            <div className="text-center mb-8">
              ...
              <p className={`text-lg ${isClassroomMode ? 'text-yellow-200' : 'text-slate-600'}`}>
                {result.accuracy >= 80 ? 'Керемет есептеу қабілеті! 🎉' : result.accuracy >= 60 ? 'Жақсы жұмыс! 👏' : 'Жаттығуды жалғастыр! 💪'}
              </p>
            </div>

            {/* Stats Grid */}
            <div className="grid grid-cols-2 gap-4 mb-8">
              <div className={`p-6 rounded-xl text-center ${isClassroomMode ? 'bg-yellow-700' : 'bg-slate-50'}`}>
                <div className={`text-4xl font-bold ${isClassroomMode ? 'text-white' : 'text-orange-600'}`}>
                  {result.accuracy}%
                </div>
                <div className={isClassroomMode ? 'text-yellow-200' : 'text-slate-600'}>Дәлдік</div>
              </div>
              <div className={`p-6 rounded-xl text-center ${isClassroomMode ? 'bg-yellow-700' : 'bg-slate-50'}`}>
                <div className={`text-4xl font-bold ${isClassroomMode ? 'text-white' : 'text-blue-600'}`}>
                  {result.correctAnswers}/{result.totalQuestions}
                </div>
                <div className={isClassroomMode ? 'text-yellow-200' : 'text-slate-600'}>Дұрыс</div>
              </div>
              <div className={`p-6 rounded-xl text-center ${isClassroomMode ? 'bg-yellow-700' : 'bg-slate-50'}`}>
                <div className={`text-4xl font-bold ${isClassroomMode ? 'text-white' : 'text-green-600'}`}>
                  {result.avgTimePerQuestion}с
                </div>
                <div className={isClassroomMode ? 'text-yellow-200' : 'text-slate-600'}>Орташа уақыт</div>
              </div>
              <div className={`p-6 rounded-xl text-center ${isClassroomMode ? 'bg-yellow-700' : 'bg-slate-50'}`}>
                ...
                <div className={isClassroomMode ? 'text-yellow-200' : 'text-slate-600'}>Жылдамдық</div>
              </div>
            </div>

            {/* Difficulty Breakdown */}
            <div className={`p-6 rounded-xl mb-8 ${isClassroomMode ? 'bg-yellow-700' : 'bg-slate-50'}`}>
              ...
              <div className="grid grid-cols-3 gap-4">
                {(['easy', 'medium', 'hard'] as DifficultyLevel[]).map(level => (
                  <div key={level} className="text-center">
                    ...
                    <div className={isClassroomMode ? 'text-yellow-200' : 'text-slate-600'}>
            ...
            {result.challengeCompleted && (
              <div className={`p-6 rounded-xl mb-8 ${result.challengeCorrect ? 'bg-green-100' : 'bg-orange-100'}`}>
                <div className="text-center">
                  <span className="text-4xl">{result.challengeCorrect ? '🏆' : '🎯'}</span>
                  <p className={`font-bold mt-2 ${result.challengeCorrect ? 'text-green-700' : 'text-orange-700'}`}>
                    Челлендж: {result.challengeCorrect ? 'Жеңіс! ⚡ +10 бонус!' : 'Келесі жолы!'}
                  </p>
                </div>
              </div>
            )}
            ...
            <div className="grid grid-cols-2 gap-4">
              <button
                onClick={startGame}
                className="py-4 bg-gradient-to-r from-yellow-500 to-orange-500 text-white font-bold text-xl rounded-xl hover:opacity-90 transition-all flex items-center justify-center gap-2"
              >
